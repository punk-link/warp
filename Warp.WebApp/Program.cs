...
void AddTelemetry(WebApplicationBuilder webApplicationBuilder)
{
    ...
    {
        tracing.ConfigureResource(configure =>configure.AddService(webApplicationBuilder.Configuration["ServiceName"]!))
    ...
    var openTelemetryApiKey = webApplicationBuilder.Configuration["OpenTelemetry:ApiKey"];
    if (!string.IsNullOrWhiteSpace(openTelemetryApiKey))
        builder.Services.Configure<OtlpExporterOptions>(options =>
        {
            options.Headers = $"x-otlp-api-key={openTelemetryApiKey}";
        });
