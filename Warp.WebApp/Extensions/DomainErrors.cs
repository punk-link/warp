...
public static class DomainErrors
{
    ...
    public static DomainError VaultTokenFileNotFound(string tokenFile)
        => new(LogEvents.VaultTokenFileNotFound, string.Format("Vault token file '{0}' was not found; set PNKL_VAULT_TOKEN manually.", tokenFile));

    public static DomainError VaultTokenFileIsEmpty(string tokenFile)
        => new(LogEvents.VaultTokenFileIsEmpty, string.Format("Vault token file '{0}' is empty; set PNKL_VAULT_TOKEN manually.", tokenFile));

    public static DomainError VaultTokenLoadedFromFile(string tokenFile)
        => new(LogEvents.VaultTokenLoadedFromFile, string.Format("Vault token successfully loaded from file '{0}'.", tokenFile));

    public static DomainError LocalConfigurationIsInUse()
        => new(LogEvents.LocalConfigurationIsInUse);

    public static DomainError EndToEndTestsConfigurationIsInUse()
        => new(LogEvents.EndToEndTestsConfigurationIsInUse);
    ...
    public static DomainError ConsulKvKeyNotFound(string storageName)
        => new(LogEvents.ConsulKvKeyNotFound, string.Format("Startup error: Consul KV key '{0}' was not found. Verify the key exists and the service name and environment are configured correctly.", storageName));

    public static DomainError ConsulKvInvalidJson(string storageName)
        => new(LogEvents.ConsulKvInvalidJson, string.Format("Startup error: Consul KV key '{0}' returned null or invalid JSON.", storageName));

    public static DomainError ConsulConfigLoaded(string storageName, int keyCount, string configData)
        => new(LogEvents.ConsulConfigLoaded, string.Format("Consul KV key '{0}' loaded {1} key(s). Values: {2}.", storageName, keyCount, configData));
    ...
    public static DomainError MaliciousImageDetected(Guid imageId, Guid entryId)
        => new(LogEvents.MaliciousImageDetected, string.Format("Image '{0}' was flagged as malicious by GuardDuty and has been excluded from entry '{1}'.", imageId, entryId));

    public static DomainError AllImagesMalicious()
        => new(LogEvents.AllImagesMalicious);

    public static DomainError MalwareScanTimeout(Guid imageId)
        => new(LogEvents.MalwareScanTimeout, string.Format("Malware scan for image '{0}' did not complete within the timeout period. The image will be treated as clean.", imageId));

    public static DomainError MalwareScanError(Guid imageId, string errorMessage)
        => new(LogEvents.MalwareScanError, string.Format("Malware scan for image '{0}' failed. Details: '{1}'.", imageId, errorMessage));
