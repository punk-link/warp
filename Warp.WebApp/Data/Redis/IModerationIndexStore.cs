...
public interface IModerationIndexStore
{
    ...
    /// </summary>
    /// <param name="member">The N-formatted GUID member key produced by <see cref="Services.CacheKeyBuilder.BuildModerationMemberKey"/>.</param>
    /// <param name="cancellationToken">Token used to observe cancellation requests.</param>
    public Task Remove(string member, CancellationToken cancellationToken);

    /// <summary>
    /// Returns up to <paramref name="take"/> N-formatted GUID member keys whose scheduled time is at or before <paramref name="maxScoreUtc"/>.
    /// </summary>
